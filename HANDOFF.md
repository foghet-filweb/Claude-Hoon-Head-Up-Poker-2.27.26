...
1. Eyre reconnect logic (openSSE):
...
2. Face-down community card placeholders (renderCommunityCards):
...
Full deployment sequence for glob/room.html changes:
  ...
  2. Copy to pier: cp repo/glob/room.html ~/bes/fhloston-poker/glob/
  ...
  7. If docket already saw this docket-0 and failed: bump version+[0 1 N]
     to force docket to retry the fetch on next commit.
  8. Restart python3 HTTP server if it died:
...
On onerror (after N retries to same channel):
  - Generate new uid
  - Reset eventId to 1
  - await eyrePut([]) — open new channel
  - Re-subscribe to /game and /chat
  - openSSE() with new channel
