...
## Project
Fhloston Poker — Heads-up Texas Hold'em on Urbit. Two ships (~nec / ~bes), Hoon backend + vanilla JS frontend.
...
1. State one success criterion first
2. Hoon sessions don't touch JS and vice versa
3. Session ends in regression → hard revert before closing, no exceptions
4. Revert command: `git reset --hard HEAD`
...
6. Update `desk.docket-0` in repo AND both pier desks:
   ...
   ```
   If docket already committed this docket-0 and failed to fetch:
   bump `version+[0 1 N]` to force a retry on next commit.
7. Ensure glob server is running:
   ```
